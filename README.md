...
You may configure Identity using either a configuration file named `.env`,
environment variables, or a combination of both. Environment variables are prefixed with `DELIVC_`, and will always have precedence over values provided via file.

### Top-Level

```
DELIVC_SITE_URL=https://example.delivc.com/
```
...
The shared secret with an operator (usually Delivc) for this microservice. Used to verify requests have been proxied through the operator and
...
Controls what endpoint Delivc can access this API on.
...
```
DELIVC_DB_DRIVER=mysql
DELIVC_DATABASE_URL=root@localhost/delivc
```

`DELIVC_DB_DRIVER` - `string` **required**

Chooses what dialect of database you want. Must be `mysql`.

`DELIVC_DATABASE_URL` - `string` **required**

Connection string for the database.

`DELIVC_DB_NAMESPACE` - `string`
...
```
LOG_LEVEL=debug # available without DELIVC prefix (exception)
DELIVC_LOG_FILE=/var/log/go/delivc_identity.log
```
...
```
DELIVC_JWT_SECRET=supersecretvalue
DELIVC_JWT_EXP=3600
DELIVC_JWT_AUD=netlify
```
...
```
DELIVC_EXTERNAL_GITHUB_CLIENT_ID=myappclientid
DELIVC_EXTERNAL_GITHUB_SECRET=clientsecretvaluessssh
```
...
```
DELIVC_SMTP_HOST=smtp.mandrillapp.com
DELIVC_SMTP_PORT=587
DELIVC_SMTP_USER=smtp-delivery@example.com
DELIVC_SMTP_PASS=correcthorsebatterystaple
DELIVC_SMTP_ADMIN_EMAIL=support@example.com
DELIVC_MAILER_SUBJECTS_CONFIRMATION="Please confirm"
```
...
Identity exposes the following endpoints:
