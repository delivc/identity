...
func (c contextKey) String() string {
	return "delivc oauth api context key " + string(c)
}
...
const (
	...
	delivcIDKey             = contextKey("delivc_id")
...
func getClaims(ctx context.Context) *DelivcClaims {
	token := getToken(ctx)
	if token == nil {
		return nil
	}
	return token.Claims.(*DelivcClaims)
}
...
// withDelivcID adds the provided request ID to the context.
func withDelivcID(ctx context.Context, id string) context.Context {
	return context.WithValue(ctx, delivcIDKey, id)
}

// getDelivcID reads the request ID from the context.
func getDelivcID(ctx context.Context) string {
	obj := ctx.Value(delivcIDKey)
	if obj == nil {
		return ""
	}
